...
class Repository:
    ...
    def get_articles_without_digest(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        youtube_videos = (
            ...
            .filter(YouTubeVideo.transcript.isnot(None), YouTubeVideo.transcript != "__UNAVAILABLE__")
            .all()
        )
        ...
        anthropic_articles = self.session.query(AnthropicArticle).filter(AnthropicArticle.markdown.isnot(None)).all()
    ...
    def get_recent_digests(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        digests = (
            self.session.query(Digest).filter(Digest.created_at >= cutoff_time).order_by(Digest.created_at.desc()).all()
        )
